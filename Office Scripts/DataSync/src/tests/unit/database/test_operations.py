...
def test_db(tmp_path):
    ...
    db_ops.create_table('TestTable', columns, primary_key=['Record_ID'])
...
def test_create_schema_version_table(test_db):
    """Test creation of schema version tracking table."""
    # Create the schema version table
    test_db.create_schema_version_table()
    
    # Verify table exists
    assert test_db.table_exists("schema_versions")
    
    # Verify columns
    columns = test_db.get_table_columns("schema_versions")
    expected_columns = {
        'version_id', 'migration_name', 'applied_at', 'description',
        'checksum', 'is_success', 'error_message', 'backup_path'
    }
    assert set(columns) == expected_columns

def test_create_database_backup(test_db):
    """Test database backup creation."""
    # Create a backup
    backup_path = test_db.create_database_backup("test_migration")
    
    # Verify backup file exists
    assert os.path.exists(backup_path)
    
    # Verify backup is logged in schema_versions
    test_db.create_schema_version_table()
    query = "SELECT * FROM schema_versions WHERE migration_name = 'test_migration'"
    result = test_db.execute_query(query).fetchone()
...
def test_verify_backup(test_db):
    """Test backup verification."""
    # Create a backup
    backup_path = test_db.create_database_backup("test_migration")
    
    # Verify valid backup
    assert test_db.verify_backup(backup_path)
    
    # Test with non-existent file
    assert not test_db.verify_backup("nonexistent.accdb")
    
    # Test with invalid file
    invalid_path = Path(backup_path).parent / "invalid.accdb"
    with open(invalid_path, 'w') as f:
        f.write("This is not a valid database file")
    assert not test_db.verify_backup(str(invalid_path))

def test_restore_from_backup(test_db):
    """Test database restoration from backup."""
    # Create initial data
    test_db.insert_record("TestTable", {
    ...
    # Modify data
    test_db.update_record("TestTable", 
                         {'Description': 'Modified'},
                         {'Record_ID': 1})
    
    # Restore from backup
    assert test_db.restore_from_backup(backup_path)
    
    # Verify data is restored
    result = test_db.execute_query(
        "SELECT Description FROM TestTable WHERE Record_ID = 1"
    ).fetchone()
    assert result.Description == 'Original'

def test_calculate_schema_checksum(test_db):
    """Test schema checksum calculation."""
    # Get initial checksum
    initial_checksum = test_db._calculate_schema_checksum()
    
    # Add a new table
    test_db.create_table("NewTable", {"TestColumn": "TEXT"})
    
    # Get new checksum
    new_checksum = test_db._calculate_schema_checksum()
    
    # Checksums should be different
    assert initial_checksum != new_checksum

def test_backup_with_invalid_path(test_db):
    """Test backup creation with invalid path."""
    # Try to create backup in non-existent directory
    test_db.db_path = Path("/nonexistent/path/db.accdb")
    
    with pytest.raises(DatabaseError):
        test_db.create_database_backup("test_migration")

def test_restore_with_invalid_backup(test_db):
    """Test restore with invalid backup file."""
    with pytest.raises(DatabaseError):
        test_db.restore_from_backup("nonexistent.accdb")

def test_multiple_backups(test_db):
    """Test creating multiple backups."""
    # Create several backups
    backup_paths = []
    for i in range(3):
        backup_path = test_db.create_database_backup(f"migration_{i}")
        backup_paths.append(backup_path)
        
    # Verify all backups exist
    for path in backup_paths:
        assert os.path.exists(path)
        assert test_db.verify_backup(path)
    
    # Verify all backups are logged
    for i, path in enumerate(backup_paths):
        query = f"SELECT * FROM schema_versions WHERE migration_name = 'migration_{i}'"
        result = test_db.execute_query(query).fetchone()
        assert result is not None
